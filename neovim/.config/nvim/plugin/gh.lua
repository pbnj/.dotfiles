	...
	if opts.fargs[1] == "repo" and opts.fargs[2] == "clone" then
		local function do_clone(input)
			...
			if not org or not repo then
				vim.notify(
					"Could not parse org/repo from: " .. input,
					vim.log.levels.ERROR
				)
				return
			end
			local target = vim.fs.joinpath(
				vim.fn.expand("~/Projects/github.com"),
				org,
				repo
			)
		...
		if opts.fargs[3] then
			...
			Snacks.input(
				{ prompt = "GitHub repo (url or org/repo): " },
				function(input)
...
local function gh_workflow(args)
	require("snacks").terminal(
		gh({ "workflow", args }),
		{ auto_close = false, interactive = true }
	)
end
...
vim.keymap.set(
	...
	{ desc = "[G]H [R]un [D]ownload artifacts", noremap = true, silent = true }
)
