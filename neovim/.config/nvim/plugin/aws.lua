local aws_profile = function(arglead)
	return vim.iter(vim.fn.systemlist({
		"rg",
		"\\[profile (.*)\\]",
		"-or",
		"$1",
...
vim.api.nvim_create_user_command("AWSIAMUsers", function(opts)
	...
	local sso_account_id = vim.trim(vim.fn.system({
		"aws",
		"configure",
		"get",
		"sso_account_id",
...
vim.api.nvim_create_user_command("AWSIAMPolicies", function(opts)
	...
	require("snacks").picker({
		...
		finder = function()
			...
			return vim.iter(policies.Policies)
				:map(function(policy)
					return {
		...
		preview = function(ctx)
			...
			local cmd_policy_version = {
				...
				ctx.item.arn,
				"--version-id",
				decoded.Policy.DefaultVersionId,
			}
...
vim.api.nvim_create_user_command("AWSIAMRoles", function(opts)
	...
	require("snacks").picker({
		...
		preview = function(ctx)
			ctx.preview:reset()
			local cmd_get_role =
				{ "aws", "iam", "get-role", "--role-name", ctx.item.text }
...
vim.api.nvim_create_user_command("AWS", function(opts)
	local cmd =
		string.format("awe --no-cli-pager --cli-auto-prompt %s", opts.args)
