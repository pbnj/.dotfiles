		...
		sources = {
			...
			files = {
				...
				actions = {
					...
					delete_file = function(picker, _)
						for _, item in
							ipairs(picker:selected({ fallback = true }))
						do
			...
			gh_repos = {
				...
				finder = function(_, ctx)
					return vim.iter(vim.json.decode(vim.fn.system({
						"gh",
						"search",
						"repos",
						"--owner",
				...
				actions = {
					clone_or_sync = function(picker, item)
						picker:close()
						local clone_path = vim.fn.expand(
							"~/Projects/github.com/" .. item.fullname
						)
						if
							vim.fn.isdirectory(
								string.format("%s/.git", clone_path)
							) == 1
						then
							...
							local cmd =
								{ "gh", "repo", "clone", item.url, clone_path }
							Snacks.terminal(cmd, {
								win = {
									wo = { winbar = vim.fn.join(cmd, " ") },
								},
			...
			filetypes = {
				...
				layout = { preset = "vscode" },
				finder = function()
					return vim.iter(vim.fn.getcompletion("", "filetype"))
			...
			treesitter_languages = {
				...
				finder = function()
					...
					return vim.iter(ts.get_available())
						:map(function(lang)
							local is_installed = installed[lang] == true
							return {
								text = lang
									.. (is_installed and " (installed)" or ""),
								language = lang,
			...
			aws_console = {
				...
				format = "text",
				finder = function()
					local aws_config = vim.json.decode(vim.system({
						vim.o.shell,
						vim.o.shellcmdflag,
						string.format(
							"cat %s | jc --ini",
						...
						:wait().stdout)
					return vim.iter(aws_config)
						:filter(function(profile)
							return string.match(profile, "%d")
						end)
						:map(function(profile)
							local sso_account_id =
								aws_config[profile].sso_account_id
							local sso_account_alias =
								aws_config[profile].sso_account_alias
							local sso_role_name =
								aws_config[profile].sso_role_name
							local sso_account_url =
								aws_config[profile].sso_account_url
			...
			scalr_workspaces = {
				...
				finder = function()
					local cache_file =
						vim.fn.expand("~/.cache/scalr/workspaces.json")
					local workspaces = {}
					...
					if #workspaces == 0 then
						...
						vim.fn.writefile(
							{ vim.json.encode(workspaces) },
							cache_file
						)
					end
					return vim.iter(workspaces)
						:map(function(ws)
							return {
								text = ws.name,
								name = ws.name,
...
vim.keymap.set({ "t", "n", "i" }, "<c-\\><c-u>", function()
	Snacks.terminal({ "pkg_up" }, { auto_close = false })
